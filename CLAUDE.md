...
│   ├── next.svg          # Next.js wordmark
│   ├── vercel.svg        # Vercel wordmark
...
- **Tailwind v4 import syntax**: `globals.css` uses `@import "tailwindcss"` — NOT the v3 directives (`@tailwind base`, `@tailwind components`, `@tailwind utilities`). Do not use v3 syntax.
...
The following defaults were inherited from `create-next-app` and should be updated as the project matures:

- **`src/app/layout.tsx` metadata** — `description` is still `"Generated by create-next-app"`. Update it to reflect the actual product.
- **`README.md`** — Contains the generic Next.js boilerplate README. Replace with project-specific documentation when the product takes shape.
